...
<<<<<<< HEAD
    ...
    const isValidQuery = (query) => {
      ...
      // Reject if only repeated characters (e.g. "aaa", "kkkkk")
      if (/^([a-z])\1{2,}$/.test(cleaned)) return false
    
      // Reject strings without vowels (common in spam/gibberish)
    ...
    const sendMessage = async () => {
      if (!input.trim() || !isValidQuery(input)) {
        setNotification({ type: 'error', message: 'Please enter a meaningful cooking question.' })
        return
      }
        ...
        {messages.map((m, i) => (
            ...
            {m.recipes?.length > 0 && (
                ...
                {m.recipes.map((r, idx) => (
                    ...
                    <span><strong>Vrijeme pripreme:</strong> {r.ready_in_minutes} min</span>
                    <span>• {r.servings} porcija</span>
                    </div>


                    <div className="recipe-actions">
                    <button className="recipe-button view" onClick={() => setModalRecipe(r)}>Show</button>
                    <button className="recipe-button save" onClick={() => saveRecipe(r)}>Save</button>
                    </div>
                    </div>
                ))}
                </div>
