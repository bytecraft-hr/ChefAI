...
function PreferencesPage() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8000/settings', {
      ...
      .catch(err =>
        console.error('Greška kod dohvaćanja postavki:', err)
      )
  }, [])
  ...
  return (
    <div className="preferences-grid">
          ...
          {allergensList.map(item => (
            <label key={item}>
              <input
                ...
                onChange={() =>
                  toggle(selectedAllergens, setSelectedAllergens, item)
                }
              />
              {item}
            </label>
      ...
      <section>
        ...
        <p>
          Dodaj stvari koje voliš – koristit ćemo ih za preporuke recepata.
        </p>
        <div
          className="pantry-tags"
          style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}
